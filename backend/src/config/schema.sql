...
CREATE TABLE page_orders (
    ...
    o_state VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (
        o_state IN (
            'pending',
            'completed',
            'failed'
...
CREATE TABLE print_orders (
    ...
    sided VARCHAR(10) NOT NULL CHECK (
        sided IN ('one-sided', 'two-sided')
    ),
    paper_size VARCHAR(5) NOT NULL CHECK (
        paper_size IN ('A4', 'A3', 'A5')
    ),
    paper_orientation VARCHAR(10) NOT NULL CHECK (
        paper_orientation IN ('portrait', 'landscape')
    ),
...
ADD CONSTRAINT fk_user_page_order FOREIGN KEY (user_id) REFERENCES users (id);

ALTER TABLE print_orders
ADD CONSTRAINT fk_user_print_order FOREIGN KEY (user_id) REFERENCES users (id);

ALTER TABLE print_orders
ADD CONSTRAINT fk_document_print_order FOREIGN KEY (document_id) REFERENCES documents (id);

ALTER TABLE print_orders
ADD CONSTRAINT fk_printer_print_order FOREIGN KEY (printer_id) REFERENCES printers (id);